...
import {APPLICATION_KEYS} from 'bedrock-app-key';
...
bedrock.events.on('bedrock.init', async () => {
  ...
  for(const [name, {id, serviceType, keys}] of APPLICATION_KEYS.entries()) {
    if(name === 'app') {
      // only create zcap clients for service keys, all other keys but "app"
      continue;
    }
...
export async function hasAvailable({id, serviceType, resources} = {}) {
  ...
  const {controller, usage} = await _getUsage({id, serviceType});
...
async function _getUsage({id, serviceType}) {
  ...
  const {data: {controller, usage, serviceId}} = response;

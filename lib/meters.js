...
bedrock.events.on('bedrock.init', async () => {
  const {clients, operationUsageCache} = config['meter-usage-reporter'];
  ...
  for(const [, client] of Object.entries(clients)) {
    ...
    ZCAP_CLIENTS.set(client.id, new ZcapClient({
      agent: httpsAgent,
...
bedrock.events.on('bedrock.exit', async () => {
  ...
  try {
    ...
    logger.error('Error during meter reporting shutdown.', {error: e});
  }
});
...
export async function upsert({meterCapability, serviceType} = {}) {
  ...
  // validate `url` as starting with an origin value from the configured
  // allow list
  _checkMeterServiceAllowList({url});

  // get meter ID (url format: <meterId>/usage)
  const id = url.substr(0, url.length - '/usage'.length);
...
export async function hasAvailable({meterCapability, resources} = {}) {
  ...
  const {id, controller, usage} = await _getUsage(
    {meterCapability});
  ...
  return {
    meter: {id, controller, usage},
    hasAvailable: storage <= availableStorage &&
...
async function _report({record, signal}) {
  ...
  // report meter usage
  await _sendUsage({meterCapability, usage, record});
...
async function _sendUsage({meterCapability/*, usage, record*/}) {
  ...
  // FIXME: use `ZCAP_CLIENTS` + meterCapability
...
async function _getUsage({meterCapability}) {
  ...
  const client = ZCAP_CLIENTS.get(meterCapability.controller);
  if(!client) {
    throw new BedrockError(
      `Meter capability controller "${meterCapability.controller}" is not ` +
      'a configured client.',
      'NotAllowedError',
      {url, httpStatusCode: 403, public: true});
  }
  const response = await client.read({capability: meterCapability});
  const {data: {id, controller, usage}} = response;
  return {id, controller, usage};
}

 ...
 */
import {config, util} from 'bedrock';
import 'bedrock-server';
const cc = util.config.main.computer();

config['meter-usage-reporter'] = {
  // `clients` are for dev use only, must be overwritten in deployments
  clients: {
    // this client is only used for testing, other named clients must be added
    // by the individual services that use this module
    test: {
...
// ensure clients is overridden in deployments
config.ensureConfigOverride.fields.push('meter-usage-reporter.clients');
